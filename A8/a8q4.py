...
def can_evolve(book, source, target):
  '''Take a source and target pokemon and determine if the source pokemon can evolve into the target pokemon.
  Inputs:
    book: pokemon book dictionary containing info aboutt pokemon evolutions
    source: name of source pokemon
    target: name of target pokemon
  Returns:
    True if source pokemon can evolve into target pokemon, False otherwise
  '''
  # base case 1: source pokemon not a key in the book
  if source not in book:
    return False
  # base case 2: targett pokemon is in the source pokemon's evolution list
  if target in book[source]:
    return True
  # recursive case:
  else:
    # iterate over all immediate evolutions of source pokemon
    for pokemon in book[source]:
      if can_evolve(book, pokemon, target): # immediate evolution of source pokemon becomes the new source pokemon
        return True
  return False # no evolution path exists
