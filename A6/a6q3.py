...
def closest_to_zero(num1, num2, num3):
  ...
  Returns:  the integer which is closest to 0;
            if more than one integer is closest, return the integer that is positive'''
  ...
  for i in num2,num3:
    if abs(i) < closest[0]: # If current argument is closer to 0, update closest tuple
      ...
      # Update closest if the current argument is larger (i.e. closest[1] is negative and i is positive)
      if i > closest[1]:  
        closest = (abs(i), i)
